...
// contents unchanged.
TEST(ThrowingCtorRecovery, PutCtorThrowRestoresFreeList) {
    ...
    ASSERT_THROW(cache.put(2, std::move(incoming)), std::runtime_error);
    ThrowingValue::shouldThrow = false;
...
TEST(ThrowingCtorRecovery, EmplaceCtorThrowRestoresFreeList) {
    ThrowingValue::shouldThrow = false;
    LRUCache<int, ThrowingValue> cache(2);
    cache.emplace(1, 10);
    EXPECT_EQ(cache.size(), 1u);
    ...
    ThrowingValue::shouldThrow = true;
    ASSERT_THROW(cache.emplace(2, 20), std::runtime_error);
    ThrowingValue::shouldThrow = false;
...
TEST(ThrowingCtorRecovery, RepeatedThrowCyclesKeepPoolConsistent) {
    LRUCache<int, ThrowingValue> cache(3);
    cache.emplace(0, 0);

    for (int i = 1; i <= 50; ++i) {
        ThrowingValue::shouldThrow = true;
        ASSERT_THROW(cache.emplace(1000 + i, i), std::runtime_error);
